...
export function SalesTable({ records, isLoading, onEdit, onDelete }: SalesTableProps) {
  return (
        <div className="sales-table-wrap">
            <table className="sales-table">
                ...
                <tbody>
                    {isLoading ? (
                        <tr><td colSpan={9}>불러오는 중입니다.</td></tr>
                    ): records.length === 0 ? (
                        ...
                        records.map((record) => (
                            <tr key={record.saleId}>
                                ...
                                <td>{formatWon(record.totalPrice ?? record.qty * record.price)}</td>
                                <td>{record.paymentMethod ?? ""}</td>
                                <td>
                                    <Button type="button" className="btn-edit" onClick={() =>
                                        onEdit(record.saleId)}>
                                        수정
                                    </Button>
                                    <Button type="button" className="btn-delete" onClick={() =>
                                        onDelete(record.saleId)}>
                                        삭제
                                    </Button>
                                </td>
                            </tr>
